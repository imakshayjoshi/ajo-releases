...
export function generateUniversalServers(item) {
  if (!item) return [];
  const raw = [];
  const declared = Array.isArray(item.players) ? item.players : Array.isArray(item.player) ? item.player : item.players || item.player ? [item.players || item.player] : [];
  raw.push(...declared);
  ...
  return raw.flatMap((entry, index) => {
    ...
    return [{ id: entry?.id || `source-${index + 1}`, name: entry?.name || entry?.translator || `Source ${index + 1}`, url, source: type, type, quality: entry?.quality || null, headers: entry?.headers || {}, provider: entry?.provider || item.provider || item.network || null }];
  });
}
