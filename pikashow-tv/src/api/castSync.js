...
export class CastSyncEngine {
  ...
  connect() {
    ...
    try {
      ...
      this.ws.onclose = () => {
        ...
        // We keep the cast link alive unconditionally and rely on the heartbeat
        // to detect a genuinely dead session rather than page visibility.
  ...
  sendToTV(payload, options = {}) {
    ...
    if (payload.type === 'WATCHLIST_SYNC' && Array.isArray(payload.items)) {
      return this.broadcast({
        type: 'WATCHLIST_SYNC',
