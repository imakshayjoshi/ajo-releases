...
/**
 * Returns clean, ad-free embed URL for any YouTube ID or URL
 */
export function getAdFreeYouTubeEmbedUrl(input) {
  if (!input) return '';
  let videoId = String(input).trim();

  // Extract 11-char ID from full URLs (watch?v=, youtu.be/, shorts/, embed/)
  ...
  // Fallback: if input is an embed URL already
  if (videoId.startsWith('http') && videoId.includes('/embed/')) {
    return videoId;
  }

  return `https://www.youtube-nocookie.com/embed/${videoId}?autoplay=1&playsinline=1&controls=1&modestbranding=1&rel=0&iv_load_policy=3&fs=1&enablejsapi=1`;
}
