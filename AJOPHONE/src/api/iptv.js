...
const CACHE_KEY = 'ajo_iptv_cache_v2';
...
export function normalizeChannelItem(channel, index = 0) {
  ...
  const item = {
    ...
    players: [{ name: channel.quality || 'Auto', url: channel.url, source: 'hls', quality: channel.quality || null, headers: channel.headers || {} }]
  };
...
export async function getIPTVChannels() {
  ...
  for (const result of results) {
    ...
    for (const channel of parseM3U(result.value)) {
      const normalized = normalizeChannelItem(channel, items.length);
  ...
  if (items.length) localStorage.setItem(CACHE_KEY, JSON.stringify({ savedAt: Date.now(), items }));
  return items;
}

export async function getJioTVServerChannels(serverHost) {
  const host = String(serverHost || localStorage.getItem(JIOTV_KEY) || '').replace(/\/$/, '');
